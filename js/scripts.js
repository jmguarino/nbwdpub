(function($) {
    ...
    $("#map").gmap3({
        marker:{     
        address:"333 Country Club Dr, Santa Rosa, CA 95401", 
    ...
    /* This plugin is broken... off by 1 error when the value is 58 but not 59? Not worth debugging
    $('.counter').counterUp({
        delay: 8,
        time: 1400
    }); */
...
var namesArr; //store all names in DB guest list
var party; //store names of members of users party when autocomplete input is submitted

//Populate autocomplete names
$(function() {
...
$(function() {
    $.getJSON("../php/db_getCounters.php", function(data) {

        $("#attendingCount").html(data.attendingCount);
        $("#waitingCount").html(data.leftCount);

        var today = new Date();
        var weddingDay = new Date("6/25/2016");
        var timeDiff = Math.abs(weddingDay.getTime() - today.getTime());
        var diffDays = Math.ceil(timeDiff / (1000 * 3600 * 24));

        $("#daysCount").html(diffDays);
    });
});

//Set RSVP options based on user's party
$(function () {
    $( "#rsvpForm" ).submit(function( event ) {
        //Keeps the form from double-posting resulting in double party name rows
        ...
        if($.inArray(name, namesArr) != -1){
            ...
            $.post("php/db_getParty.php", {passedName: name}).done(function (data) {
                console.log(data);
                party = $.parseJSON(data);
                console.log("Data length:"+data.length);
                console.log("Party length:"+party.length);
                console.log(party);
                $(" #guestRows ").append("<input type='hidden' name='partySize' value='"+party.length+"'>");
                $(" #guestRows ").append("<input type='hidden' name='partyNames' value='"+party+"'>");
                for(i = 0; i<party.length; i++) {
                    $(" #guestRows ").append("<tr>\
                        <td>"+party[i]+"</td>\
                        <td>\
                        <label class='radio-inline active'><input type='radio' name='guest"+i+"' value='1' checked='checked'>will be</label>\
                        <label class='radio-inline'><input type='radio' name='guest"+i+"' value='0'>will not be</label>\
                        </td>\
                        <td>attending.</td>\
                        </tr>");
                }
...
//RSVP for users and update DB
$(function () {
    $("#rsvpChoices").submit(function( event) {

        console.log("clicked rpsv submit");

        var values = $(this).serialize();

        console.log("Values: "+values);

        $.post("php/db_setRSVP.php", values).done(function (data) {
            var success = 1;
            ...
            if(success==1){
                $("#partyRSVP").addClass("hidden");
                $("#rsvpSuccess").removeClass("hidden");
                ...
                alert("Something strange happened so an email has been sent to Justin.");

                console.log("error: "+values)
                var error_vals = JSON.stringify(values);

                 $.post("php/ej.php", {error: error_vals});
            }
        });
